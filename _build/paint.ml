...
type shape = 
  ...
  | Points of {color: color; points: point list; thickness: int}
...
let with_params (g: gctx) (c: color) (t: int) : gctx =
  let g_colored = with_color g c in
  Gctx.with_thickness g_colored t
  ...
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Ellipse e ->
        ...
        draw_ellipse (with_params g e.color e.thickness) center rx ry
    end in
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
          ...
          | PointMode -> 
            (* Create a new Points record with the first point and save to 
            preview *)
            paint.preview <- Some (Points {color=paint.color; points=[p]; 
              thickness=paint.thickness})
            (* Start the ellipse tracking *)
      ...
      (begin match paint.mode with 
          | LineEndMode p1 -> 
            paint.preview <- Some (Line {color=paint.color; p1=p1; p2=p; 
              thickness=paint.thickness})
          | PointMode ->
            (* Get the existing points from the preview *)
            let points_list = 
              begin match paint.preview with 
              | Some (Points ps) -> ps.points
              | _ -> []
              end in
            (* Update the preview with the new point added to the list *)
            paint.preview <- Some (Points {color=paint.color; 
              points = p :: points_list; thickness=paint.thickness})
          | EllipseEndMode p1 -> 
            (* Update the preview ellipse *)
    ...
    | MouseUp ->
      ...
      (begin match paint.mode with 
          | LineEndMode p1 ->
            ...
            paint.preview <- None
          | PointMode -> 
            (* Get the final list of points from preview *)
            begin match paint.preview with 
            | Some (Points ps) -> 
              (* Insert all points into history deque *)
            ...
            paint.preview <- None
          | EllipseEndMode p1 ->
            (* Save the ellipse and clear the preview *)
            Deque.insert_tail 
...
(* Create the line, point, ellipse, and thick buttons *)
let (w_line, _, nc_line) = button "Line"
let (w_point, _, nc_point) = button "Point"
let (w_ellipse, _, nc_ellipse) = button "Ellipse"
let (w_thick, vc_thick) = checkbox false "Thick"

(* Create a slider with a max thickness of 15 and a width of 80 pixels *)
let (w_thick_slider, vc_thick_slider) = slider 15 80

(* Create a label so the user knows what the slider is for *)
let (w_slider_label, _) = label " Thickness: "

(* Add listeners to change the mode when clicked *)
...
;; vc_thick.add_change_listener (fun (checked: bool) ->
    if checked then 
      (* If checked, get whatever value the slider is currently at *)
      paint.thickness <- max 2 (vc_thick_slider.get_value ())
    else 
      (* If unchecked, make thickness 1 *)
      paint.thickness <- 1)

(* The slider only updates state if the box is checked *)
;; vc_thick_slider.add_change_listener (fun v ->
  if vc_thick.get_value () then
    paint.thickness <- max 2 v
  else 
    (* If box is unchecked, moving slider does nothing to the actual paint 
    thickness *)
    ())
...
let mode_toolbar : widget =
  hlist [w_undo; spacer; w_line; spacer; w_point; spacer; w_ellipse; spacer; 
    w_thick; spacer; w_slider_label; w_thick_slider]
